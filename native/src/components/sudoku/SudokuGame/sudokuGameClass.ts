...
export default class Sudoku {
  ...
  solve(isGenerating: boolean) {
    return this.solveUltimately(isGenerating);
  }

  solveUltimately(isGenerating: boolean) {
    if (this.isSolved()) {
      return true;
    }
    ...
    while (allowedNumbers.length > 0) {
      if (isGenerating && new Date().getTime() - solveTimeout.getTime() > 50) {
        return false;
      }
      const value = allowedNumbers.shift() as number;
      this.set(x, y, value);

      try {
        if (this.solveUltimately(isGenerating)) {
          return true;
        }
      } catch (err) {
        // Nothing
