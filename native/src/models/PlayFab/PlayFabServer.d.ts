declare module PlayFabServerModule {
  export interface IPlayFabServer {
    ...
    // Adds users to the set of those able to update both the src data, as well as the set of users in the group. Only users
    // in the group (and the server) can add new members. src Groups are designed for sharing data between a very small
    // number of players, please see our guide:
    // https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/addsharedgroupmembers
    ...
    // Requests the creation of a src group object, containing key/value pairs which may be updated by all members of the
    // group. When created by a server, the group will initially have no members. src Groups are designed for sharing data
    // between a very small number of players, please see our guide:
    // https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/createsharedgroup
    CreateSharedGroup(
    ...
    // Deletes a src group, freeing up the src group ID to be reused for a new group. src Groups are designed for
    // sharing data between a very small number of players, please see our guide:
    // https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/deletesharedgroup
    DeleteSharedGroup(
    ...
    // Retrieves data stored in a src group object, as well as the list of members in the group. The server can access all
    // public and private group data. src Groups are designed for sharing data between a very small number of players,
    // please see our guide: https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/getsharedgroupdata
    GetSharedGroupData(
    ...
    // Removes users from the set of those able to update the src data and the set of users in the group. Only users in the
    // group can remove members. If as a result of the call, zero users remain with access, the group and its associated data
    // will be deleted. src Groups are designed for sharing data between a very small number of players, please see our
    // guide: https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/removesharedgroupmembers
    ...
    // Adds, updates, and removes data keys for a src group object. If the permission is set to Public, all fields updated
    // or added in this call will be readable by users not in the group. By default, data permissions are set to Private.
    // Regardless of the permission setting, only members of the group (and the server) can update the data. src Groups are
    // designed for sharing data between a very small number of players, please see our guide:
    // https://docs.microsoft.com/gaming/playfab/features/social/groups/using-src-group-data
    // https://docs.microsoft.com/rest/api/playfab/server/src-group-data/updatesharedgroupdata
...
declare module PlayFabServerModels {
  ...
  export interface AddSharedGroupMembersRequest extends PlayFabModule.IPlayFabRequestCommon {
    ...
    PlayFabIds: string[];
    // Unique identifier for the src group.
  ...
  export interface CreateSharedGroupRequest extends PlayFabModule.IPlayFabRequestCommon {
    // Unique identifier for the src group (a random identifier will be assigned, if one is not specified).
    SharedGroupId?: string;
  }
  ...
  export interface GetSharedGroupDataRequest extends PlayFabModule.IPlayFabRequestCommon {
    // If true, return the list of all members of the src group.
    GetMembers?: boolean;
    // Specific keys to retrieve from the src group (if not specified, all keys will be returned, while an empty array
